...
function SectorFlowTableFallback() {
  return (
    <div className="rounded-xl border border-glass bg-glass border-glass px-4 py-8 backdrop-blur-sm">
      <div className="text-center text-xs text-ink-3">板块排行加载异常</div>
    </div>
  );
}
...
export function SectorFlowTable({ topInflows, topOutflows }: SectorFlowTableProps) {
  ...
  const allSectors = useMemo(() => {
    const combined = [
      ...(Array.isArray(topInflows) ? topInflows : []).map((s) => ({ ...s, _group: 'inflow' as const })),
      ...(Array.isArray(topOutflows) ? topOutflows : []).map((s) => ({ ...s, _group: 'outflow' as const })),
    ];
  ...
  const Th = ({ colKey, label, className }: { colKey: SortKey; label: string; className?: string }) => (
    <th
      className={cn(
        'cursor-pointer select-none px-3 py-2 text-[11px] font-semibold text-ink-3 transition-colors duration-200 hover:text-ink focus-visible:ring-1 focus-visible:ring-primary/40 focus-visible:outline-none',
        className,
      )}
      role="columnheader"
      aria-sort={sortKey === colKey ? (sortDir === 'desc' ? 'descending' : 'ascending') : 'none'}
      tabIndex={0}
  ...
  const categoryLabel = (category?: string) => {
    ...
    if (c === 'concept') return '概念';
    if (c === 'region') return '地域';
    return c;
  };
  ...
  try {
    return (
      <motion.div
        ...
        className="rounded-xl border border-glass bg-glass border-glass"
      >
        <div className="flex items-center justify-between border-b border-glass px-4 py-2.5">
          <div className="flex items-center gap-2">
            <span className="text-sm font-semibold text-ink">板块资金流向排行</span>
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="搜索板块…"
              aria-label="搜索板块"
              className="w-32 rounded-lg border border-glass bg-glass-md border-glass py-1 pl-7 pr-2 text-xs text-ink placeholder-ink-3 outline-none transition-colors duration-200 focus:border-primary/30 focus-visible:ring-1 focus-visible:ring-primary/40"
            />
          </div>
        </div>

        <div className="overflow-x-auto">
          <table className="w-full" role="table" aria-label="板块资金流向排行">
            <thead>
            ...
            >
              {sorted.map((s, i) => (
                <motion.tr
                  ...
                  className={cn(
                    'transition-colors duration-200 hover:bg-glass-sm even:bg-glass-subtle',
                  )}
                >
                  ...
                  <td className={cn(cellClass, 'font-medium')}>
                    <div className="flex items-center gap-2">
                      ...
                      {categoryLabel(s.category) ? (
                        <span className="ml-0.5 rounded bg-glass-subtle px-1 py-0.5 text-[10px] font-medium text-ink-3">
                          {categoryLabel(s.category)}
                        </span>
                      ) : null}
                      <span
              ...
              {sorted.length === 0 && (
                <tr>
                  <td colSpan={COL_COUNT} className="px-3 py-8 text-center text-xs text-ink-3">
                    {filter ? '无匹配板块' : '暂无板块数据'}
                  </td>
                </tr>
              )}
    ...
    console.error('SectorFlowTable render error:', err);
    return <SectorFlowTableFallback />;
  }
}
